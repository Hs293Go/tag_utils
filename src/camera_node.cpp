...
int main(int argc, char *argv[])
{
    ...
    std::string camera_info_url = "file://" + ros::package::getPath("tag_utils") + "/camera_info/" + camera_name + ".yaml";
    if (nh.getParam("camera_info_url", camera_info_url))
    {
        camera_info_url = "file://" + fs::absolute(camera_info_url).generic_string();
    }
    ...
    std::stringstream pipe_stream;

    pipe_stream << "nvarguscamerasrc ! video/x-raw(memory:NVMM), width=(int)" << capture_width << ", height=(int)" << capture_height
                    << ", format=(string)NV12, framerate=(fraction)" << framerate << "/1 ! nvvidconv flip-method="
                    << flip_method << " ! video/x-raw, width=(int)" << display_width << ", height=(int)" << display_height
                    << ", format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink";

    std::string pipeline = pipe_stream.str();
    ...
    image_transport::Publisher camera_image_publisher = it.advertise("/" + camera_name + "/image_rect", 1);

    ros::Publisher camera_info_pub = nh.advertise<sensor_msgs::CameraInfo>("/" + camera_name + "/camera_info", 1);
